...
async def process_single_question(
    ...
    logging.info(
        f"Calling process_single_question with question: '{context.question}' and expert_type: '{context.expert_type}'"
    )
    try:
        ...
        related_questions = await generate_related_questions(
            client, context.question, context.expert_type
        )
        ...
        results = await process_questions(client, all_questions, context.expert_type)
        stop_animation(task, len(fetch_message))

        display_results(results)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_results(OUTPUT_DIR, context.question, results, timestamp)
...
async def main():
    """Main function to run the Question Assistant application."""
    try:
        # Setup logging
        ...
        while True:
            ...
            await process_single_question(
                client,
                QuestionContext(question, expert_type),
                start_animation,
                stop_animation,
            )

            if not get_user_choice():
                print("\nThank you for using the Question Assistant!")
                break
